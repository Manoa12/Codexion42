...
if command -v valgrind > /dev/null 2>&1; then
	...
	if grep -q "definitely lost: 0 bytes" /tmp/codexion_valgrind.log \
		&& grep -q "indirectly lost: 0 bytes" /tmp/codexion_valgrind.log; then
		ok "Valgrind : aucune fuite memoire (definitely/indirectly lost = 0)"
	else
